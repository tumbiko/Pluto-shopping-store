...
const SearchBar = () => {
  ...
  const [desktopOpen, setDesktopOpen] = useState(false) // md+
  ...
  // click outside to close for both mobile & desktop
  useEffect(() => {
  ...
  return (
    <div ref={wrapperRef} className="relative flex items-center">
      ...
      {/* SMALL (sm): keep inline search input if you want it visible on small screens (>= sm AND < md) */}
      <div className="hidden sm:flex md:hidden items-center bg-white border rounded-full px-3 py-1 w-64 lg:w-70">
      ...
      {/* MD & UP: show only an icon that toggles the dropdown */}
      ...
      {/* MD+ dropdown panel (shows when icon toggled) */}
      {desktopOpen && (
        // anchor to the right of the wrapper so it opens from the icon and doesn't stretch left
        <div className="hidden md:block absolute top-full right-0 mt-2 translate-y-2 z-50">
          <div className="min-w-[450px] max-w-sm w-80 bg-white border rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="max-h-60 overflow-y-auto">
              {query ? (
                loading ? (
                  ...
                  results.map((product) => (
                    <Link key={product._id} href={`/product/${product.slug.current}`} className="flex items-center p-3 hover:bg-gray-100" onClick={clearAndClose}>
                      {product.imageUrl && <Image src={product.imageUrl} alt={product.name} width={40} height={40} className="w-10 h-10 object-cover rounded mr-3" />}
                      <div>
      ...
      {/* MOBILE expanded search (unchanged) */}
      {mobileOpen && (
